<template>
  <div class="container bg-secondary py-2 my-3">
    <div class="card col-md-10 my-5 mx-auto">
      ...
      <div class="card-body">
        <form id="form" :action="this.product_store" method="POST" enctype="multipart/form-data">
          <input type="hidden" name="_token" id="token" />

          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="product_name" id="label_product_name">產品名稱</label>
            </div>
          ...
          <div class="alert alert-danger" v-if="jpErrors['product_name'] != null">
            {{ jpErrors["product_name"][0] }}
          </div>

          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="gender" id="label_gender">商品歸屬性別</label>
            </div>
          ...
          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="price" id="label_price">單價</label>
            </div>
          ...
          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="quantity" id="label_quantity">商品總數</label>
            </div>
          ...
          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="category" id="label_category">商品類型</label>
            </div>
          ...
          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="style" id="label_style">商品風格</label>
            </div>
          ...
          <div class="input-group mb-2">
            <div class="input-group-prepend">
              <label class="input-group-text" for="is_sold" id="label_is_sold">販賣許可</label>
            </div>
            ...
            <div class="input-group-text justify-content-between">
              ...
              />
              <input class="mr-4" type="radio" id="determine" name="is_sold" value="1" v-else />

              <label class="input-group-text mr-2" for="deny">稍後</label>
              <input type="radio" id="deny" name="is_sold" value="0" checked v-if="this.jpOlds['is_sold'] == 0" />
          ...
          <div class="input-group mb-2">
            <button class="btn-primary btn-lg btn-block my-2 col-5 mx-auto" id="addImageBtn" v-on:click="addImage()">
              增加圖片
            </button>
            <button
          ...
          <button type="submit" class="btn btn-primary btn-lg btn-block my-2" id="submitBtn">
            submit
          </button>
        </form>
      </div>
...
export default {
  ...
  mounted() {
    ...
    document.getElementById("gender").addEventListener("change", (e) => {
      document.getElementById("style").innerHTML = "<option disabled selected>請先選取商品類型</option>";
      console.log(this.search_category_and_style_route);
      var searchtext =
        this.search_category_and_style_route + "/gender?gender=" + document.getElementById("gender").value;
      console.log(searchtext);
      axios
        .get(searchtext, {})
        .then((res) => {
          console.log(res.data);
          var category_text = "<option disabled selected>請選擇商品類型</option>";

          document.getElementById("category").innerHTML = "";
          res.data.forEach((element) => {
            category_text += "<option value=" + element["category"] + ">" + element["category"] + "</option>";
          });
      ...
      axios
        ...
        .then((res) => {
          ...
          if (res.data.length == 0) {
            ...
            var style_text = "<option disabled selected>請選擇商品風格</option>";

            document.getElementById("style").innerHTML = "";
            res.data.forEach((element) => {
              style_text += "<option value=" + element["style"] + ">" + element["style"] + "</option>";
            });
  ...
  methods: {
    addImage() {
      ...
      img_html += "<label class='input-group-text' for='img_" + imgs.length + "'>圖片" + imgs.length + "</label>";
      img_html += "</div>";
      ...
      img_html +=
        ...
        imgs.length +
        "' />";
      img_html += "<label class='custom-file-label' for='img_" + imgs.length + "'>選擇圖片</label>";
      img_html += "</div>";
      img_html += "</div>";
      ...
      document.getElementById("img_" + imgs.length).addEventListener("change", (e) => {
