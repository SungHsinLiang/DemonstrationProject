		...
		<div class="d-flex my-4">
			...
			<div class="text-center col-6 m-auto">
				<form :action="route" method="post" id="purchase_cart_form">
					<h2 class="font-weight-bold" id="product_id">
						Product Id : {{ product["id"] }}
					</h2>
					<br />
					<h2 class="font-weight-bold">{{ product["product_name"] }}</h2>
					<br />
					<h2 id="total_quantity">
...
export default {
	...
	mounted() {
		...
		for (const [key, value] of Object.entries(this.jpImgs)) {
			if (key == "img_main" || key == "img_main_thunbnail" || value == "empty")
				continue;
			imgs_content_text +=
				"<img src='" + value + "' alt=" + key + " class='mb-4' />";
		}
